...
"""
    ...
    for (img_dict, anno_dict_list) in imgs_anns:
        record = {}
        if 'datasets' in img_dict["file_name"]:
            record["file_name"] = str(img_dict["file_name"])
        else:
...
def load_coco_json_eval(json_file, image_root, dataset_name=None, extra_annotation_keys=None):
    ...
    timer = Timer()
    json_file_t = PathManager.get_local_path('datasets/coco/annotations/instances_val2017.json')
    with contextlib.redirect_stdout(io.StringIO()):
        coco_api = COCO(json_file_t)
    if timer.seconds() > 1:
        logger.info("Loading {} takes {:.2f} seconds.".format(json_file_t, timer.seconds()))
