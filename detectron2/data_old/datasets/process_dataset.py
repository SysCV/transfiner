...
"""
        ...
        id_map_old = {v: i for i, v in enumerate(cat_ids)}

        id_map = {}
        voc_idxs = []
        for cnt, c in enumerate(sorted(cats, key=lambda x: x["id"])):
            if c['name'] in CLASS_NAMES:
                voc_idxs.append(cnt)
            id_map[c['id']] = cnt
        print(voc_idxs)
        meta.thing_dataset_id_to_contiguous_id = id_map
        assert len(voc_idxs) == 20
    ...
    print('L131 coco.py Anns 0:', anns[0])
    ...
    for jdex, (img_dict, anno_dict_list) in enumerate(imgs_anns):
        ...
        #print('img w/h:', img_dict["width"], img_dict["height"])
        objs = []
        ...
        if len(seg_list) > 0:
            for index, seg in enumerate(seg_list):
                ...
                #print('bitmask non zeoro:', np.count_nonzero(bitmask.astype(int)))
                #print(dirname+'/'+ img_dict["file_name"].split('.')[0]+'_'+str(index)+'.jpg')
                #im = Image.fromarray(bitmask)
                #im.save(dirname+'/'+ img_dict["file_name"].split('.')[0]+'_'+str(index)+'.jpg')
                #cv2.imwrite(dirname+'/'+ img_dict["file_name"].split('.')[0]+'_'+str(index)+'.jpg', np.array(bitmask))
        ...
        for index, obj in enumerate(objs):
            ...
            #im = Image.fromarray(box_mask.astype(bool))
            #im.save(dirname+'/'+ img_dict["file_name"].split('.')[0]+'_'+str(index)+'_mask'+'.jpg')
        ...
        for index1, a_box in enumerate(box_list):
            ...
            for index2, b_box in enumerate(box_list):
                if index1 != index2:
                    ...
                    if iou > 0.05:
                        #print(index1,':', index2,':', iou)
                        union_mask = np.multiply(box_mask_list[index1], bitmask_list[index2])
                        #print('union_mask shape:', union_mask.shape)
                        #print('union_mask nonzero num:', np.count_nonzero(union_mask))
                        union_mask_whole += union_mask
            
            #print('union mask whole non zeoro:', np.count_nonzero(union_mask_whole.astype(int)))
            ...
            if union_mask_non_zero_num > 20:
                ...
                #bitmask = polygons_to_bitmask(record["annotations"][index1]['bg_object_segmentation'], img_dict["height"], img_dict["width"])
                #im = Image.fromarray(bitmask)
                #im.save(dirname+'/'+ img_dict["file_name"].split('.')[0]+'_'+str(index1)+'_mask_union_binary_light_new_final_end'+'.jpg')

            #im = Image.fromarray(union_mask_whole.astype(bool))
            #im.save(dirname+'/'+ img_dict["file_name"].split('.')[0]+'_'+str(index1)+'_mask_union'+'.jpg')

        #print('len(box_list):',len(box_list))
        #print('len(seg_list):',len(seg_list))
        #print('len(bitmask_list):',len(bitmask_list))

        dataset_dicts.append(record)
        if jdex >= 1000:
            break
    rate = float(sum_co_box) / float(sum_box)
    print('rate for co box:', rate)
...
if __name__ == "__main__":
    ...
    #output_file = 'instances_train_2017_transform_slight.json'
