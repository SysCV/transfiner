...
@torch.jit.unused
def mask_rcnn_loss(pred_mask_logits: torch.Tensor, pred_mask_logits_uncertain: torch.Tensor, x_hr: torch.Tensor, x_hr_l: torch.Tensor, x_hr_ll: torch.Tensor, x_c: torch.Tensor, transfomer_encoder: torch.nn.Module, instances: List[Instances], vis_period: int = 0):
    """
    ...
    mask_uncertain_bool_lg_l = (pred_mask_logits_uncertain_lg_l.detach() >= 0.875)
    ...
    for box_i in range(len(number_pts)):
        ...
        if uncertain_feats_s.size()[0] < 5 or uncertain_feats_s_l.size()[0] < 5 or uncertain_feats_s_ll.size()[0] < 5:
    ...
    x_c_pos = pos_embed(x_c).flatten(2)[:LIMIT][valid_box_pos].permute(2, 0, 1)
...
class BaseMaskRCNNHead(nn.Module):
    ...
    def forward(self, x, instances: List[Instances]):
        ...
        """
        x, x_uncertain, x_hr, x_hr_l, x_hr_ll, x_c, encoder = self.layers(x)

        if self.training:
            loss_masks, loss_mask_uncertains, loss_mask_refine = mask_rcnn_loss(
                x, x_uncertain, x_hr, x_hr_l, x_hr_ll, x_c, encoder, instances, self.vis_period)
            ...
            mask_uncertain_bool = (pred_mask_logits_uncertain.detach() >= 0.0001) # ori is 0.5, main contribution
            # print('mask_uncertain_bool shape:', mask_uncertain_bool.shape)
            mask_uncertain_bool_lg = (pred_mask_logits_uncertain_lg.detach() >= 0.125).squeeze(1) # ori is 0.5
            mask_uncertain_bool_lg_l = (pred_mask_logits_uncertain_lg_l.detach() >= 0.5).squeeze(1).cpu() # ori is 0.1 0.5 also final 1 is 0.05
            # print('mask_uncertain_bool_lg_l:', mask_uncertain_bool_lg_l.shape) current eval is 0.5 and iterate 5pixels
            # 0.1 is 36.6 what about 0.5?
            
            # print('mask_uncertain_bool_lg_l shape:', mask_uncertain_bool_lg_l.shape)
            if mask_uncertain_bool_lg_l.shape[0] > 0: # current is using 0.5
                kernel = torch.ones(5, 5)
                mask_uncertain_bool_lg_l = dilation(mask_uncertain_bool_lg_l.unsqueeze(1).float(), kernel).squeeze(1).bool()

            # kernel = np.ones((3,3), np.uint8)
            # kernel = np.ones((8,8), np.uint8)
            # mask_uncertain_bool_lg_l = np.float32(mask_uncertain_bool_lg_l.numpy())
            # for m_i in range(mask_uncertain_bool_lg_l.shape[0]):
            #     #modify_c = cv2.dilate(mask_uncertain_bool_lg_l[m_i], kernel, iterations=4)
            #     modify_c = cv2.dilate(mask_uncertain_bool_lg_l[m_i], kernel, iterations=1)
            #     #outputs_masks_per_image[m_i][0][modify_r] = modify_c[modify_r]
            #     mask_uncertain_bool_lg_l[m_i] = modify_c

            # mask_uncertain_bool_lg_l = torch.from_numpy(mask_uncertain_bool_lg_l).bool().to(mask_uncertain_bool_lg.device)
            ...
            # print('mask_uncertain_bool_lg shape:', mask_uncertain_bool_lg.shape)
            ...
            for i_i in range(2):
                ...
                # print('select num hr:', select_num_hr)

                if select_num_hr > 0: # switch for modification
                ...
                if select_num_hr_l > 0: # switch for modification
            ...
            # print('pred_mask_logits_bool:', pred_mask_logits_bool.shape)
            # print('mask_uncertain_bool_lg_l:', mask_uncertain_bool_lg_l.shape)
            # pred_mask_logits_bool = mask_uncertain_bool_lg_l.unsqueeze(1)
...
class MaskRCNNConvUpsampleHead(BaseMaskRCNNHead):
    ...
    def layers(self, x_list):
        ...
        return mask, mask_uncertain, x_hr, x_hr_l, x_hr_ll, x_c, self.encoder
